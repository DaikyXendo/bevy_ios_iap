    ...
    do {
      ...
      let result =
        ...
        case .success(let transactionResult):
          IosIapPurchaseResponse.success(
            try convert_transaction(transactionResult.unsafePayloadValue))
        case .userCancelled:
      ...
      purchase_processed(
        request,
        IosIapPurchaseResponse.purchase_error(
          convert_purchase_error(error), error.localizedDescription))
    } catch let error as StoreKitError {
      purchase_processed(
        request,
        IosIapPurchaseResponse.storekit_error(
          convert_storekit_error(error), error.localizedDescription))
    } catch {
...
public func ios_iap_transaction_finish(request: Int64, transaction_id: UInt64) {
  Task {
    do {
      for await t in Transaction.unfinished {
        ...
        if transaction.id == transaction_id {
          ...
          transaction_finished(
            request, IosIapTransactionFinishResponse.finished(try convert_transaction(transaction)))
          return
        }
      }
      ...
      transaction_finished(
        request, IosIapTransactionFinishResponse.error(error.localizedDescription))
    }
  }
}
...
func convert_product(_ product: (Product)) async throws -> IosIapProduct {
  ...
  var rust_product = IosIapProduct.new(
    product.id, product.displayPrice, product.displayName, product.description,
    Double(truncating: product.price as NSNumber), type)
  ...
  if let sub = product.subscription {
    ...
    for s in status {
      ...
      rust_status.push(
        value: IosIapSubscriptionStatus.new(renewalState, try convert_transaction(transaction)))
    }

    let subscription = IosIapSubscriptionInfo.new(
      sub.subscriptionGroupID, period, try await sub.isEligibleForIntroOffer, rust_status)
...
public func convert_purchase_error(_ error: Product.PurchaseError) -> IosIapPurchaseError {
  ...
  case .paymentMethodBindingConfigurationRequired: IosIapPurchaseError.purchase_not_allowed()
  @unknown default: IosIapPurchaseError.purchase_not_allowed()
  }
}
...
public func convert_transaction(_ transaction: (Transaction)) throws -> IosIapTransaction {
  ...
  let env =
    if transaction.environment == AppStore.Environment.xcode {
  ...
  if #available(iOS 17.0, *) {
    let store = IosIapStorefront.storefront(
      transaction.storefront.id, transaction.storefront.countryCode)

    IosIapTransaction.add_storefront(t, store)

    let reason =
      if transaction.reason == Transaction.Reason.purchase {
  ...
  if let currency = transaction.currency {
    IosIapTransaction.add_currency(
      t, IosIapCurrency.new(currency.identifier, currency.isISOCurrency))
  }
